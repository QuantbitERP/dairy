...
from dairy.milk_entry.report.milk_ledger.milk_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	if filters.get('show_stock_ageing_data'):
		...
		item_wise_fifo_queue =  FIFOSlots(filters).generate()
...
def get_item_warehouse_map(filters, sle):
	...
	for d in sle:
		...
		if d.posting_date < from_date:
			...
			qty_dict.fat += fat_diff
			qty_dict.snf += snf_diff
		elif d.posting_date >= from_date and d.posting_date <= to_date:
			if flt(qty_diff, float_precision) >= 0:
				qty_dict.in_qty += qty_diff
				# qty_dict.in_val += value_diff
			if flt(fat_diff, float_precision) >= 0:
				qty_dict.in_fat += fat_diff
			if flt(snf_diff, float_precision) >= 0:
				qty_dict.in_snf += snf_diff
			else:
				qty_dict.out_qty += abs(qty_diff)
				# qty_dict.out_val += abs(value_diff)
				qty_dict.out_fat += abs(fat_diff)
				qty_dict.out_snf += abs(snf_diff)
...
def validate_filters(filters):
	if not (filters.get("item_code") or filters.get("warehouse")):
		sle_count = flt(frappe.db.sql("""select count(name) from `tabMilk Ledger Entry`""")[0][0])
		if sle_count > 500000:
